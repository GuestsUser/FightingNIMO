...
public class EasingTest : MonoBehaviour
{
    ...
    void Turn()
    {
        ...
        // timeがターンする地点への到着時間以下の時時(まだターンする地点に到着していない間)
        ...
        {
            // ターン地点から次の地点までを補完
            value = easing(duration, time, length, !symbol, old_pos, max[1], true);
        }
    ...
    float easing(float duration, float time, float length,bool symbol,float source,float max, bool turn) 
    {
        ...
        // ターン時かどうかで変わる　ターン時はSinカーブ上で言うと　1 なので -1 して 0 から 1のカーブになるようにする 
        if (turn)
        {
